...
export class EsriMapService {
  ...
  private esriMapDataSources = new BehaviorSubject<esriMapData[]>([]);
  ...
  getColores(index,cantNiveles){
    var listScheme=[];
    palette.listSchemes('cb-sequential').forEach(function(e){listScheme.push(e.scheme_name)});
...
/*
  obtenerDatosMapaTematico(){
    ...
    const url =`http://192.168.34.16:8877/poccrim/delitos/?groupby=ccdd&anio=2017`;
  ...
  formatearDato(datos){
    var data = datos.map(x=> { return  {'codigo':x._id.codigo_mapa , 'valor': x.delitos} });
    return data
  }
